...
# Breathing Reminder GNOME Shell Extension Installer/Updater

set -e

EXTENSION_UUID="breathing-reminder@varunbpatil"
...
if [ -d "$EXTENSION_DIR" ]; then
    echo "Updating Breathing Reminder GNOME Shell Extension..."
    ...
    echo "Installing Breathing Reminder GNOME Shell Extension..."
fi

# Create extension directory
mkdir -p "$EXTENSION_DIR"

# Copy extension files (exclude install.sh, .git, README, LICENSE)
echo "Copying extension files..."
find . -maxdepth 1 -type f -not -name "install.sh" -not -name ".git*" -not -name "README.md" -not -name "LICENSE" -exec cp {} "$EXTENSION_DIR/" \;
find . -maxdepth 1 -type d -not -name "." -not -name ".git" -exec cp -r {} "$EXTENSION_DIR/" \;

# Compile gschema
if [ -f "$EXTENSION_DIR/schemas/org.gnome.shell.extensions.breathing-reminder.gschema.xml" ]; then
